			...
			if user.getDirty() > 0 then
				...
				SetTimeout(90000, function()
					if transfer == "doing it" then
						...
						TriggerClientEvent("pNotify:SendNotification", source, {text = "You have cleaned <span style='color:lime'>$</span><span style='color:white'>"..tonumber(dm).."</span> and now have <span style='color:lime'>$</span><span style='color:white'>"..savedUser.getMoney().."</span>",type = "error",queue = "left",timeout = 20000,layout = "centerRight"})
					end
				end)
			else
