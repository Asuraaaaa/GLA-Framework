	...
	if(#args < 3) then
		...
		if(GetPlayerName(tonumber(args[2])) ~= nil and tostring(args[3]) ~= nil)then
			...
			if ranks[rank:lower()] ~= nil then
				...
				TriggerClientEvent("pNotify:SendNotification", -1, {text = "<b style='color:red'>Alert</b> <br><span style='color:lime'>".. GetPlayerName(tonumber(args[2])) .."</span> has been accepted. <br> Congratulations on joining the EMS Division!",type = "error",queue = "left",timeout = 10000,layout = "bottomRight"})
	...
	if(not args[2]) then
		...
		if(GetPlayerName(tonumber(args[2])) ~= nil)then
			...
			if ems[identifier] ~= nil then
				ems[identifier] = nil
				saveData(ems,"ems.txt",main)
				TriggerClientEvent("pNotify:SendNotification", -1, {text = "<b style='color:red'>Alert</b> <br><span style='color:lime'>".. GetPlayerName(tonumber(args[2])) .."</span> has been fired. <br> They are no longer a paramedic!",type = "error",queue = "left",timeout = 10000,layout = "bottomRight"})
