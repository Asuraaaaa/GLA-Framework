	...
	if identifier and license then
		...
		if playerCount >= 32 then
			...
			if steamcheck == "steam" then
			    ...
			    if age then
			    	print("[ANTI-CHEESE]::"..identifier.."("..name..") tried to connect but (account age < week)!")
					...
					for k,v in pairs(bantable["Anticheat"]) do
						if v.identifier == identifier or v.license == license then
							acban = true
							print("[ANTI-CHEESE]::"..identifier.."("..name..") tried to connect but was previously banned!")
							banreason = identifier.."("..name.."), you are banned! Reason: "..v.reason..""
							break
						end
					end
					if acban then
						...
						local nban = false
						print("[ANTI-CHEESE]::"..name.." has cleared ban check for cheating")
						for k,v in pairs(bantable["Normal"]) do
							if v.identifier == identifier or v.license == license then
								...
								if currentTime > v.duration then
									...
									nban = true
									banreason = identifier.."("..name.."), you are banned! Expires: "..showdate.month.."/"..showdate.day.."/"..showdate.year.." at "..showdate.hour..":"..showdate.min..":"..showdate.sec.." Reason: "..v.reason..""
									break
								end
							end
						end
...
function getAccountAge(hexId, callback)
    local apiKey = "4C04BE82F4AF41C0D856217597A34700" -- Serenity's API Key
...
function BanCheater(reason,source)
	...
	saveData(bantable, "bans.txt", path.bans)
	log("bans", GetPlayerName(source).." | "..identifier.." | "..license.." | Forever | "..reason.." | ANTI-CHEESE")
			PerformHttpRequest('https://discordapp.com/api/webhooks/414864181125316608/JWEebHFxLnRcm_ye2rJGTsRhv5iwoVCmA2QuBGOY9yGHXxnE6jjqinUBDby9Dl2YgjS7', function(err, text, headers) end, 'POST', json.encode(
			{
