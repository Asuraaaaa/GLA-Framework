...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if IsControlJustPressed(1, 166) and carshop_menu == false and carrental_menu == false and engineon == true then --- F5 (Open)
...
function MainMenu()
    Menu.SetupMenu("main_menu","~y~G~w~L~g~A")
    ...
    Menu.addOption("main_menu", function()
        if GetItemQuantity(100) > 0 then
        if (Menu.Option("Phone"))then
            TriggerEvent("phone:open")
            player_menu = false 
        end
        else
        	Menu.Option("No Phone")
            --DisplayHelpText("~r~~bold~ You Don't have a phone!") -- doesnt even work properly, but without lines 770-772 it's anal and refuses to work.
            player_menu = true
        end
    end)
    Menu.addOption("main_menu", function()
    ...
    Menu.addOption("main_menu", function()
    	if(Menu.Option("Player Info")) then
    		PlayerMenu()
    	end
    	end)
    Menu.addOption("main_menu", function()
...
function VehMenu()
    if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
        local veh = GetVehiclePedIsUsing(GetPlayerPed(-1), false)
        if (GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1)) then
            if exports.police:getIsInService() == true then
            ...
            elseif exports.paramedic:getIsInService() == true then
                Menu.SetupMenu("vehicle_menu","Vehicle Controls")
                Menu.Switch("main_menu","vehicle_menu")
                Menu.addOption("vehicle_menu", function()
                    if(Menu.Bool("Engine", enginetoggled,"~r~OFF","~g~ON",function(cb)    enginetoggled = cb end)) then
	        ...
	        if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
	            ...
	            if IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) then
	            	...
	            	if not car_found then
	                	SetEntityMaxSpeed(veh, 320/2.236936)
	                end
	            end
	        end
        end
...
function PlayerMenu()
	Menu.SetupMenu("player_info","Player Info")
    Menu.Switch("main_menu","player_info")
	Menu.addOption("player_info", function()
        if(Menu.Option("Health: "..(GetEntityHealth(GetPlayerPed(-1)))))then
        end
    end)
    Menu.addOption("player_info", function()
    	if(Menu.Option("Armour: "..(GetPedArmour(GetPlayerPed(-1)))))then
    	end
    end)
end
...
Citizen.CreateThread(function()
    while true do
        ...
        if NetworkIsPlayerTalking(PlayerId()) then
            drawThisTxt(0.283, 0.95, 0.25, 0.03, 0.50,"~b~ Voice : "..voip,255,255,255,255,6)
        else
...
local options = {
    {text="Buy Password",price=10000},
    {text="Blackmarket Weapons location",price=50000},
    {text="Collect product for meth location",price=15000},
    {text="Produce Meth location",price=15000},
    {text="Distribute Meth location",price=15000},
    {text="Harvest Cannabis location",price=5000},
    {text="Transform Cannabis location",price=10000},
    {text="Distribute Cannabis location",price=5000},
    {text="Distribute Cannabis in mass location",price=10000},
    {text="Money Laundering location",price=20000},
}

function InformerMenu()
    Menu.SetupMenu("main_menu","Notes")
    Menu.Switch(nil,"main_menu")
...
local informer = {
    { ['x'] = 523.18347167969, ['y'] = -858.68432617188, ['z'] = 16.515375137329 },
}
        ...
        for k,v in ipairs(informer) do
            if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 20.0)then
                ...
                if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 1.0)then
                    DisplayHelpText("Press ~INPUT_VEH_EXIT~ to ~g~Talk to the adviser.")
