...
--[[Citizen.CreateThread(function()
    while true do
    Citizen.Wait(0)
        if IsPedInAnyPoliceVehicle(GetPlayerPed(PlayerId())) or IsPedInModel(GetPlayerPed(-1), GetHashKey("ambulance")) or IsPedInModel(GetPlayerPed(-1), GetHashKey("firetruk")) or IsPedInModel(GetPlayerPed(-1), GetHashKey("riot")) then
            local veh = GetVehiclePedIsUsing(GetPlayerPed(PlayerId()), false)
            if (GetPedInVehicleSeat(veh, -1) == GetPlayerPed(PlayerId())) then
                if isInService == true or exports.paramedic:getIsInService() == true then
                else
