$(document).ready(function(){  // Mouse Controls
  ...
  function addWarrants(items) {
    for(let i in items) {
      ...
      $("#warrant-" + item.id + "").click(function(){
        $.post('http://police/warrantRead', JSON.stringify({id: item.id, timestamp: timestamp, officer_name: item.officer_name, offender_name: item.offender_name, location: item.location, notes: item.notes}));  
      })
    }
  }
  ...
  function addArrests(items) {
    for(let i in items) {
      ...
      $("#arrest-" + item.id + "").click(function(){
        $.post('http://police/viewArrest', JSON.stringify({id: item.id, timestamp: timestamp, officer_name: item.officer_name, offender_name: item.offender_name, sentenceORfine: item.sentenceORfine, sentence: '' + item.sentence + ' seconds', charges: item.charges, fine: '$' + item.fine + ''}));  
      })
    }
  }
  ...
  function addViolations(items) {
    for(let i in items) {
      ...
      $("#violation-" + item.id + "").click(function(){
        $.post('http://police/viewViolation', JSON.stringify({id: item.id, timestamp: timestamp, officer_name: item.officer_name, offender_name: item.offender_name, violations: item.violations, fine: '$' + item.fine + ''}));  
      })
    }
  }
  ...
  document.onkeyup = function (data) {
    if (data.which == 27 ) {
      $.post('http://police/close', JSON.stringify({}));
    }
  };
  ...
  $(".btnWarrants").click(function(){
      $.post('http://police/loadWarrants', JSON.stringify({}));
  });
  $(".btnMDT").click(function(){
      $.post('http://police/loadArrests', JSON.stringify({}));
  });
  $(".btnViolations").click(function(){
      $.post('http://police/loadViolations', JSON.stringify({}));
  });
  $(".btnRemove").click(function(){
    if (confirm('Are you sure you want permanently delete this warrant?') === true) {
      $.post('http://police/warrantDelete', JSON.stringify({
          id: $("#warrant-id").val(),
      }));
      setTimeout(function(){
        closeAll();
  ...
  $(".btnRemove2").click(function(){
    if (confirm('Are you sure you want permanently delete this arrest record?') === true) {
      $.post('http://police/removeArrest', JSON.stringify({
          id: $("#arrest-id").val(),
      }));
      setTimeout(function(){
        closeAll();
  ...
  $(".btnRemove3").click(function(){
    if (confirm('Are you sure you want permanently delete this violation?') === true) {
      $.post('http://police/removeViolation', JSON.stringify({
          id: $("#violation-id").val(),
      }));
      setTimeout(function(){
        closeAll();
  ...
  $(".btnNewWarrant").click(function(){
      $.post('http://police/newWarrant', JSON.stringify({}));
  });
  
  // Handle Form Submits
  $("#newWarrant-form").submit(function(e) {
      e.preventDefault();
      $.post('http://police/newWarrantSubmit', JSON.stringify({
  ...
  $(".btnNewMDT").click(function(){
      $.post('http://police/newArrest', JSON.stringify({}));
  });
  $("#new-arrest-entry-form").submit(function(e) {
      e.preventDefault();
      $.post('http://police/newArrestSubmit', JSON.stringify({
  ...
  $(".btnMDTAutofill").click(function(){
      $.post('http://police/autofillArrest', JSON.stringify({}));
  });
  $(".btnNewViolation").click(function(){
      $.post('http://police/newViolation', JSON.stringify({}));
  });
  $("#new-violation-entry-form").submit(function(e) {
      e.preventDefault();
      $.post('http://police/newViolationSubmit', JSON.stringify({
  ...
  $(".btnViolationAutofill").click(function(){
      $.post('http://police/autofillViolation', JSON.stringify({}));
  });
