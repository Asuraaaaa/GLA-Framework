...
Citizen.CreateThread(function()
    while true do
        ...
        if(isCop) then
            if(isInService) then
		        if IsControlJustPressed(1, 170) then --- F3 (Open)
		        	TriggerServerEvent('police:requestMission')
		        	GUI.maxVisOptions = 10; GUI.titleText = {255, 255, 255, 255, 1}; GUI.titleRect = {0, 250, 0, 255}; GUI.optionText = {0, 150, 0, 255, 6}; GUI.optionRect = {0, 0, 0, 190}; GUI.scroller = {0, 0, 0, 255}; titleTextSize = {0.85, 0.80}; titleRectSize = {0.16, 0.085}; optionTextSize = {0.5, 0.5}; optionRectSize = {0.16, 0.035}; menuX = 0.75; menuXOption = 0.075; menuXOtherOption = 0.050; menuYModify = 0.3000; menuYOptionDiv = 8.56; menuYOptionAdd = 0.36;
...
--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--
function MainMenu()
    Menu.SetupMenu("cop_menu","Police")
    ...
    Menu.addOption("cop_menu", function()
        if(Menu.Option("~y~Citizen"))then
        	cit()
        end
    end)
    Menu.addOption("cop_menu", function()
        if(Menu.Option("~w~Vehicle"))then
        	Vehicle()
        end
    end)
    Menu.addOption("cop_menu", function()
        if(Menu.Option("~o~Placeables"))then
        	Placeables()
        end
    end)
    Menu.addOption("cop_menu", function()
        if(Menu.Option("~r~MDT Area"))then
        	Warrents()
        end
    end)
end
...
function Vehicle()
    ...
    Menu.addOption("cop_menu_vehicle", function()
    	if(Menu.Option("Check Plate"))then
			...
			local entityWorld = GetOffsetFromEntityInWorldCoords(GetPlayerPed(-1), 0.0, 50.0, 0.0)
    ...
    Menu.addOption("cop_menu_vehicle", function()
        if(Menu.Option("Impound vehicle"))then
			...
			if DoesEntityExist(closestVeh) then
				if exports.core_panel:GetActiveTows() > 0 then
